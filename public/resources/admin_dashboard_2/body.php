<body ng-app="adminDashboardApp" ng-controller="adminDashboardController">
  ...
  <div class="main-dash" id="kelola-buku">
    ...
    <span class="ket-dash">Dashboard / Kelola Anggota / Bendahara | <a href="<?=$Variable['URL'];?>admin/dashboard/index.php?secretaryList=show&treasurerList=hide" style="color: blue;">Sekretaris</a></span>
    <div class="tambah-buku">
      <img src="<?=$Variable['URL'];?>resources/assets/icon/add-role.png">
    </div>
    <ng-form method="POST" name="searchTreasurerForm"> 
	   <div class="main-tab">
    	  <div class="search-cat">
    	  	<i class="fa fa-search fa-2x" ng-click="searchTreasurerForm.$valid ? searchTreasurer() : errorSearchTreasurer()"></i>
            <input placeholder="Cari Bendahara Berdasarkan Nama" type="text" name="keyword" ng-model="keyword" required>
		  </div>
       </div>
    	<div ng-show="searchTreasurerForm.keyword.$touched && searchTreasurerForm.keyword.$error.required">
    ...
    <div class="list-buku">
    <?php 
    	if (empty($Variable['Data_Bendahara']) == false)
        {
        	?>
       ...
       <?php
        	for ($i=1; $i <= count($Variable['Data_Bendahara']); $i++) 
            {
    ?>		 <tr>
          		<td><?=$Variable['Data_Bendahara'][$i]['Nama_Bendahara'];?></td>
        	    <td>Bendahara</td>
          		<td><a href="javascript:void(0)" ng-click="admindelRoleTreasurer('<?=$Variable['Data_Bendahara'][$i]['ID_Bendahara'];?>')" style="color:blue;">Hapus</a></td>
          </tr>
    <?php
    	   }
     ?> 
    ...
    ?>
      	  <div class="not-table">
        	<span>Tidak ada anggota Bendahara yang dapat ditampilkan</span>
      	  </div>
    <?php
    	}
    ?>
    <div class="pagging">
    <?php
              if ($Variable['Halaman_Paging_Bendahara'] < $Variable['Total_Halaman_Bendahara'])
              {
                ?> <a href="<?=$Variable['URL'].'secretary/dashboard/index.php?secretaryList=show&treasurerList=hide&page_secretary='.($Variable['Halaman_Paging_Bendahara']+1);?>" style="color: blue;">Selanjutnya</a> <?php
              }
              else
              {
              ...
              if ($Variable['Halaman_Paging_Bendahara'] > $Variable['Total_Halaman_Bendahara'] OR $Variable['Halaman_Paging_Bendahara'] == 1)
              {

              }
              else
              {
                 ?> <a href="<?=$Variable['URL'].'secretary/dashboard/index.php?secretaryList=show&treasurerList=hide&page_secretary='.($Variable['Halaman_Paging_Bendahara']-1);?>" style="color: blue;">Sebelumnya</a> <?php
              } 
      ?>
      </div>
    </div>
 	    ...
 	    <div ng-model="getKeyword"><p style="margin-top: 15px;margin-left: 32px;width: 95%;">Hasil pencarian <strong>{{getKeyword}}</strong></p></div>
 	  	<div class="list-buku">
 	  		<table>
            	...
            	<td>Bendahara</td>
            	<td><a ng-href="javascript:void(0)" ng-click="adminDeleteRoleTreasurer(data)" style="color: blue;">Hapus</a></td>
          	  </tr>
          	</table>
 	  	</div>
 	  </div>
