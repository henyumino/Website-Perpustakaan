var adminAccountSettings = angular.module('adminAccountSettingsApp', ['chieffancypants.loadingBar', 'ngAnimate', 'ngFileUpload', 'ngImgCrop'])
.config(function(cfpLoadingBarProvider) {
    cfpLoadingBarProvider.includeSpinner = false;
  })

 adminAccountSettings.controller('adminAccountSettingsController', function($window, $scope, $http, $timeout, cfpLoadingBar, Upload) {
    ...
    $scope.adminEditAccount = function(croppedprofilepicture, profilepicturename) {
      if ($scope.profilepicture == undefined)
      {
          ...
          .then((confirm) => {
            if (confirm) {
                ...
                $http({
                        method: "post",
                        url: "http://localhost:8080/projekuas/ajax/admin_update_account_informations.php",
                        data: {
                                 id            : $scope.id,
                       ...
                       if (res['Type'] == 'success') 
                        {
                             ...
                             $scope.adminEditAccountForm.$setPristine();
                             $scope.adminEditAccountForm.$setUntouched();
                             swal({
                                    text: res['Message'],
                                    icon: res['Type'],
      ...
      else
      {
         ...
         .then((confirm) => {
            if (confirm) {
              Upload.upload({
                              url: "http://localhost:8080/projekuas/ajax/admin_update_account_informations_with_upload.php",
                              method: "POST",
          ...
          swal({
              ...
              .then((confirm) => {
                if (confirm) {
                    ...
                    $http({
                            method: "post",
                            url: "http://localhost:8080/projekuas/ajax/admin_change_password.php",
                            data: {
                                    id           : $scope.id,
                    ...
                    if (typeof res === "object") 
                    {
                      if (res['Type'] == 'success') 
                      {
                        $scope.password     = '';
                        $scope.newpassword  = '';
                        $scope.adminChangePasswordForm.$setPristine();
                        $scope.adminChangePasswordForm.$setUntouched();
                        swal(res['Title'], res['Message'], res['Type']);
                      }
                      else
                      {
