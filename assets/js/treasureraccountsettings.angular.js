var treasurerAccountSettings = angular.module('treasurerAccountSettingsApp', ['chieffancypants.loadingBar', 'ngAnimate', 'ngFileUpload', 'ngImgCrop'])
.config(function(cfpLoadingBarProvider) {
    cfpLoadingBarProvider.includeSpinner = false;
  })

 treasurerAccountSettings.controller('treasurerAccountSettingsController', function($window, $scope, $http, $timeout, cfpLoadingBar, Upload) {
    ...
    $scope.treasurerEditAccount = function(croppedprofilepicture, profilepicturename) {
      if ($scope.profilepicture == undefined)
      {
          ...
          .then((confirm) => {
            if (confirm) {
                ...
                $http({
                        method: "post",
                        url: "http://localhost:8080/projekuas/ajax/treasurer_update_account_informations.php",
                        data: {
                                 id            : $scope.id,
                       ...
                       if (res['Type'] == 'success') 
                        {
                             ...
                             $scope.treasurerEditAccountForm.$setPristine();
                             $scope.treasurerEditAccountForm.$setUntouched();
                             swal({
                                    text: res['Message'],
                                    icon: res['Type'],
                                    confirm:true,
      ...
      else
      {
         ...
         .then((confirm) => {
            if (confirm) {
              Upload.upload({
                              url: "http://localhost:8080/projekuas/ajax/treasurer_update_account_informations_with_upload.php",
                              method: "POST",
     ...
     $scope.treasurerChangePassword = function() {
          swal({
                  text: "Apakah anda sudah yakin dengan data yang anda masukkan?",
                  icon: "info",
                  buttons: true,
              ...
              .then((confirm) => {
                if (confirm) {
                    ...
                    $http({
                            method: "post",
                            url: "http://localhost:8080/projekuas/ajax/treasurer_change_password.php",
                            data: {
                                    id           : $scope.id,
                    ...
                    if (typeof res === "object") 
                    {
                      if (res['Type'] == 'success') 
                      {
                        $scope.password     = '';
                        $scope.newpassword  = '';
                        $scope.treasurerChangePasswordForm.$setPristine();
                        $scope.treasurerChangePasswordForm.$setUntouched();
                        swal(res['Title'], res['Message'], res['Type']);
                      }
                      else
                      {
