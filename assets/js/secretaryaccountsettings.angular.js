var secretaryAccountSettings = angular.module('secretaryAccountSettingsApp', ['chieffancypants.loadingBar', 'ngAnimate', 'ngFileUpload', 'ngImgCrop'])
.config(function(cfpLoadingBarProvider) {
    cfpLoadingBarProvider.includeSpinner = false;
  })

 secretaryAccountSettings.controller('secretaryAccountSettingsController', function($window, $scope, $http, $timeout, cfpLoadingBar, Upload) {
    ...
    $scope.secretaryEditAccount = function(croppedprofilepicture, profilepicturename) {
      if ($scope.profilepicture == undefined)
      {
          ...
          .then((confirm) => {
            if (confirm) {
                ...
                $http({
                        method: "post",
                        url: "http://localhost:8080/projekuas/ajax/secretary_update_account_informations.php",
                        data: {
                                 id            : $scope.id,
                       ...
                       if (res['Type'] == 'success') 
                        {
                             ...
                             $scope.secretaryEditAccountForm.$setPristine();
                             $scope.secretaryEditAccountForm.$setUntouched();
                             swal({
                                    text: res['Message'],
                                    icon: res['Type'],
      ...
      else
      {
         ...
         .then((confirm) => {
            if (confirm) {
              Upload.upload({
                              url: "http://localhost:8080/projekuas/ajax/secretary_update_account_informations_with_upload.php",
                              method: "POST",
     ...
     $scope.secretaryChangePassword = function() {
          swal({
                  text: "Apakah anda sudah yakin dengan data yang anda masukkan?",
                  icon: "info",
                  buttons: true,
              ...
              .then((confirm) => {
                if (confirm) {
                    ...
                    $http({
                            method: "post",
                            url: "http://localhost:8080/projekuas/ajax/secretary_change_password.php",
                            data: {
                                    id           : $scope.id,
                    ...
                    if (typeof res === "object") 
                    {
                      if (res['Type'] == 'success') 
                      {
                        $scope.password     = '';
                        $scope.newpassword  = '';
                        $scope.secretaryChangePasswordForm.$setPristine();
                        $scope.secretaryChangePasswordForm.$setUntouched();
                        swal(res['Title'], res['Message'], res['Type']);
                      }
                      else
                      {
